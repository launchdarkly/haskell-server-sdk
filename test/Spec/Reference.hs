...
import LaunchDarkly.Server.Reference (makeReference, makeLiteral, getError, getComponents, getRawPath)
import Test.HUnit

invalidReferences :: Test
invalidReferences = TestCase $ (do
...
validReferencesWithLeadingSlash = TestCase $ (do
  confirm "/key" "key"
  confirm "/0" "0"
...
canAccessSubcomponents = TestCase $ (do
  confirm "/key" ["key"]
  confirm "/a/b" ["a", "b"]
...
validLiteralReferences = TestCase $ (do
  confirm "name" "name"
  confirm "a/b" "a/b"
  confirm "/a/b~c" "/~1a~1b~0c"
  confirm "/" "/~1")
  where confirm ref path = let literal = makeLiteral ref
